...
_redraw()
{
  ...
  if (sec) {
    XSetForeground(dis, gc, foreground.pixel);
    XDrawString(
        dis, double_buffer, gc, 0, font_height, sec->name, strlen(sec->name));
    ...
    XDrawString(dis, double_buffer, gc,
        (strlen(sec->name) + 1) * font_info->per_char->width, font_height, data,
        strlen(data));
    ...
    price_to_pixel = linear_equation_new(
        max_value, font_height, min_value, xwa.height - font_height);
    ...
    for (uint32_t i = min_value; i <= max_value; i += pips) {
      ...
      XDrawString(dis, double_buffer, gc,
          ...
          linear_equation_eval(price_to_pixel, i), level, strlen(level));
    }
