...
static bool
_valid_segment(struct candle *cnds, struct linear_equation *eq, int start,
               int end)
{
  if (start < 0)
  {
...
analysis_support_trend(struct candle *cnds, size_t indx)
{
  ...
  while (maximum_segment > 1)
  {
    ...
    struct linear_equation *eq = linear_equation_new(
        ...
        last_confirmed - maximum_segment,
        _candle_working_value(cnds[last_confirmed - maximum_segment]));

    int confirmations = 1;

    while (_valid_segment(cnds, eq, last_confirmed - maximum_segment,
                          last_confirmed))
    {
      bool last_confirmed_changed = false;
      for (int i = (last_confirmed - maximum_segment) - (maximum_segment / 2);
           i <= (last_confirmed - maximum_segment) + (maximum_segment / 2); ++i)
      {
        if (i >= 0)
        {
          if (cnds[i].high == linear_equation_eval(eq, i))
          {
            last_confirmed = i;
            last_confirmed_price = cnds[i].high;
            last_confirmed_changed = true;
          }
          else if (_candle_working_value(cnds[i]) ==
                   linear_equation_eval(eq, i))
          {
            last_confirmed = i;
    ...
    if (confirmations >= 4)
    {
      ...
      chart_create_object_line(&(cnds[indx - 1]), (size_t)starting_index,
                               starting_price, (size_t)last_confirmed,
                               last_confirmed_price, __func__);
      break;
    }
...
struct vtable exports = {&analysis_support_trend};
